...
class SensorManager:
    ...
    def __init__(self, use_cnc_subprocess: bool = False, cnc_python_path: str = None, test_mode: bool = None):
        """
        Args:
    ...
    async def _initialize_sensors_background(self):
        ...
        # 각 센서를 비동기적으로 초기화 (연결된 것만 활성화)
        # NOTE: HikRobot 카메라는 현재 비활성화 (연결 불가 상태)
    ...
    async def collect_all_data(self) -> Dict[str, Any]:
        ...
        if self.connection_status["camera"] and "camera" in self.databases:
            try:
                ...
                if camera_data:
                    # image_available 플래그 추가 (프론트엔드에서 이미지 로드 트리거용)
